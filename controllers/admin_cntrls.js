...
const attend_Mdl = require("../models/attendance_model");
const student_Mdl = require("../models/student_model");
const studentAttend_Mdl = require("../models/studentAttendance_model");
...
const getAllStudentAttendance = async (req, res) => {
    try {
        const records = await studentAttend_Mdl.find()
            .populate("studentId", "fullName studentRoll center")
            .sort({ attendanceDate: -1 });

        const msg = records.length ? "Student attendance records fetched" : "No records found";
...
const getStudentAttendanceById = async (req, res) => {
    try {
        ...
        const records = await studentAttend_Mdl.find({ studentId }).sort({ attendanceDate: -1 });

        const msg = records.length ? "Attendance history fetched" : "No records found for this student";
        res.status(200).json(responseGenerator(true, msg, records));
    } catch (err) {
        console.error(err);
        res.status(500).json(responseGenerator(false));
...
const deleteStudentAttendance = async (req, res) => {
    try {
        ...
        const deleted = await studentAttend_Mdl.findByIdAndDelete(attendanceId);

        if (!deleted) {
            return res.status(404).json(responseGenerator(false, "Attendance record not found"));
        }

        res.status(200).json(responseGenerator(true, "Attendance record deleted", deleted));
    } catch (err) {
        console.error(err);
        res.status(500).json(responseGenerator(false));
    }
